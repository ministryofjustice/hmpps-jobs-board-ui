...
context('Sign In', () => {
  ...
  it('Add employer flow', () => {
    cy.task('getBrokerIterationEnabled').then(flagValue => {
      const expectedHeading = flagValue ? 'Manage jobs and employers' : 'Add jobs and employers'
      const employerListPage = new EmployerListPage(expectedHeading)
      ...
      employerListPage.heading().contains(expectedHeading)
    })
  })
