...
context('Sign In', () => {
  ...
  it('Validation error display', () => {
    ...
    cy.get('@nationalJobsEnabled').then(isEnabled => {
      if (isEnabled) {
        ...
        // Page errors
        jobIsNationalUpdatePage.isNationalPageErrorMessage().contains('Select if this is a national job')

        // Field errors
        jobIsNationalUpdatePage.isNationalFieldErrorMessage().contains('Select if this is a national job')
  ...
  it('Create job flow - without national jobs', () => {
    // Skip this test if national jobs is enabled
    cy.checkFeatureToggle('nationalJobs', isEnabled => {
      skipOn(isEnabled)
    })
  ...
  it('Create job flow - with national jobs', () => {
    ...
    // Job location and contract page
    const jobContractUpdatePage = new JobContractUpdatePage('Job location and contract')
    jobContractUpdatePage.headerCaption().contains('Add a job - step 3 of 6')
    ...
    // Requirements and job description page
    const jobRequirementsUpdatePage = new JobRequirementsUpdatePage('Requirements and job description')
    jobContractUpdatePage.headerCaption().contains('Add a job - step 4 of 6')
    ...
    // How to apply page
    const jobHowToApplyPage = new JobHowToApplyPage('How to apply')
    jobHowToApplyPage.headerCaption().contains('Add a job - step 5 of 6')
    ...
    const jobReviewPage = new JobReviewPage('Check your answers before adding job')
    jobReviewPage.headerCaption().contains('Add a job - step 6 of 6')
  ...
  it('Create job - change links flow - with national jobs', () => {
    // Skip this test if national jobs is not enabled
    cy.checkFeatureToggle('nationalJobs', isEnabled => {
      skipOn(!isEnabled)
    })
    ...
    jobIsNationalUpdatePage.isNationalFieldNo().click()
    jobIsNationalUpdatePage.submitButton().click()
    jobContractUpdatePage.postCodeField().clear().type('NE15 7LR')
    jobContractUpdatePage.submitButton().click()
